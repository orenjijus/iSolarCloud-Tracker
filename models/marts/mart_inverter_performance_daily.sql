...
LEFT JOIN {{ ref('dim_assets') }} da ON i.asset_id = da.asset_id
LEFT JOIN {{ ref('dim_date_generated') }} dd ON dd.date_key = DATE(i.timestamp_5min)
LEFT JOIN {{ ref('seed_metric_mapper') }} m ON i.metric_id = m.metric_id AND m.used = 'yes'
