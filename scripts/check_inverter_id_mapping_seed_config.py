...
def main() -> None:
    ...
    q = text(
        ...
        SELECT
          site_id,
          COUNT(*) AS rows_cnt,
          COUNT(inverter_no) AS inverter_no_filled,
          COUNT(NULLIF(TRIM(inverter_id::text), '')) AS inverter_id_filled
        FROM staging.seed_inverter_config
        WHERE site_id IN ('NE=54435794','NE=53771627','NE=51758766','NE=58630782')
        GROUP BY site_id
        ORDER BY site_id
        """
    )
