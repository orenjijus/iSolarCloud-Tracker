...
**Important:** Raw tables have foreign keys, but mart tables don't. This is by design and follows industry best practices.

### Quick Answer
- ✅ Raw tables (`isolarcloud_*`, `fusionsolar_*`) have foreign keys
- ❌ Mart tables do NOT have foreign keys (by design)
- ✅ Relationships tracked via dbt's `{{ ref() }}` 
- ✅ PowerBI relationships configured manually (required regardless)

### Why No FK in Marts?
- 🚀 **Fast incremental loads**: 2-5 min vs 15-20 min
- 📊 **PowerBI works the same**: Manual relationship config always required
- ✅ **Data integrity maintained**: Via LEFT JOINs and unique keys
- 🌟 **Industry standard**: Modern ELT best practice

### Full Documentation
- **Quick Reference**: See [`docs/FK_IN_DBT_QUICK_REFERENCE.md`](docs/FK_IN_DBT_QUICK_REFERENCE.md) for elevator pitch answers
- **Comprehensive Analysis**: See [`docs/FOREIGN_KEYS_IN_DBT_ANALYSIS.md`](docs/FOREIGN_KEYS_IN_DBT_ANALYSIS.md) for complete technical explanation
- **dbt Lineage**: Run `dbt docs serve` to see relationship graphs

**When challenged:** Refer to the documentation above for detailed arguments and industry references.
