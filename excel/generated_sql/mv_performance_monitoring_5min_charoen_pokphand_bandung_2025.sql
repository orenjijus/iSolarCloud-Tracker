...
-- Site: Charoen Pokphand Bandung
-- Year: 2025
-- Generated: C:\Users\Administrator\Documents\Code\MMSR API - Server MA\excel\generate_mv_performance_monitoring.py
-- ============================================================================

CREATE MATERIALIZED VIEW IF NOT EXISTS mart.mv_performance_monitoring_5min_charoen_pokphand_bandung_2025 AS
WITH 
-- ============================================================================
...
meter_pivot AS (
    ...
    WHERE m.site_name = 'Charoen Pokphand Bandung'
...
sensor_pivot AS (
    ...
    WHERE s.site_name = 'Charoen Pokphand Bandung'
...
inverter_pivot AS (
    SELECT 
        ...
        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_1_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_1_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_10_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_2_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_2_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_3_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_3_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_4_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_4_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_5_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_5_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_6_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_6_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_7_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_7_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_8_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_8_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
        END) as inverter_9_active_power_kw,

        MAX(CASE 
            WHEN i.asset_id = 'ISO_1637095_1_9_1'
            AND i.metric_name = 'inv_active_power'
            THEN i.metric_value 
    ...
    WHERE i.site_name = 'Charoen Pokphand Bandung'
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_performance_monitoring_5min_charoen_pokphand_bandung_2025_unique_timestamp 
    ON mart.mv_performance_monitoring_5min_charoen_pokphand_bandung_2025(timestamp);

CREATE INDEX IF NOT EXISTS idx_mv_performance_monitoring_5min_charoen_pokphand_bandung_2025_date 
    ON mart.mv_performance_monitoring_5min_charoen_pokphand_bandung_2025(date_key);

CREATE INDEX IF NOT EXISTS idx_mv_performance_monitoring_5min_charoen_pokphand_bandung_2025_date_timestamp 
    ON mart.mv_performance_monitoring_5min_charoen_pokphand_bandung_2025(date_key, timestamp);

-- ============================================================================
-- Grant Permissions
-- ============================================================================

GRANT SELECT ON mart.mv_performance_monitoring_5min_charoen_pokphand_bandung_2025 TO PUBLIC;

-- ============================================================================
-- Create View Wrapper
-- ============================================================================

CREATE OR REPLACE VIEW mart.vw_performance_monitoring_5min_charoen_pokphand_bandung_2025 AS
SELECT * 
FROM mart.mv_performance_monitoring_5min_charoen_pokphand_bandung_2025
ORDER BY timestamp;

GRANT SELECT ON mart.vw_performance_monitoring_5min_charoen_pokphand_bandung_2025 TO PUBLIC;
