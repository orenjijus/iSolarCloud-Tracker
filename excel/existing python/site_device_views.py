...
POINT_DESCRIPTIONS = {
    ...
    "p8000": "Phase A Voltage_V",
    "p8001": "Phase B Voltage_V",
    "p8002": "Phase C Voltage_V",
    "p8006": "Phase A Current_A",
    "p8007": "Phase B Current_A",
    "p8008": "Phase C Current_A",    
...
ISOLARCLOUD_SITE_MEASURING_POINTS = {
    "Garuda Metalindo (IKP)": {
        ...
        "meter": {
            "points": ["p8018", "p8014", "p8031", "p8033", "p8030", "p8032", "p8000", "p8001", "p8002", "p8006", "p8007", "p8008"],
            "device_type": 7
        }
    },
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if create_inverter_summary:
                # Try to import the dedicated module for inverter summary views
                try:
                    # Construct the module path
                    module_path = os.path.join(os.path.dirname(__file__), 'create_inverter_summary_views.py')
                    spec = importlib.util.spec_from_file_location("create_inverter_summary_views", module_path)
                    summary_module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(summary_module)
                    
                    # Call the create_inverter_summary_views function from the module
                    logger.info(f"Using dedicated module for creating inverter summary views: {module_path}")
                    summary_module.create_inverter_summary_views(
                        sites=sites, 
                        drop_existing=False,  # Don't drop again since we've already done that if needed
                        debug=logger.level == logging.DEBUG
                    )
                    logger.info("Successfully created inverter summary views using dedicated module")
                    
                except Exception as e:
                    logger.error(f"Error using dedicated inverter summary module: {e}")
                    logger.info("Falling back to basic inverter summary views")
                    
                    # Create simple placeholder views as a fallback
                    for site_name in site_list:
                        if site_name not in ISOLARCLOUD_SITE_MEASURING_POINTS:
                            continue
                            
                        # Create a minimal view with timestamp, device_name, and a NULL column
                        clean_site_name = site_name.replace(' ', '_').replace('(', '').replace(')', '')
                        escaped_site_name = escape_sql(site_name)
                        summary_view_name = f"{clean_site_name}_inverter_summary_data"
                        
                        # Simple SQL for fallback
                        fallback_sql = f"""
                        CREATE OR REPLACE VIEW {summary_view_name} AS
                        SELECT 
                            h.timestamp,
                            d.device_name,
                            NULL::numeric AS yield_today_kWh,
                            NULL::numeric AS total_dc_power_W,
                            NULL::numeric AS total_active_power_W,
                            NULL::numeric AS total_reactive_power_var
                        FROM 
                            isolarcloud_historical_data h
                        JOIN 
                            isolarcloud_devices d ON h.device_ps_key = d.device_ps_key
                        ...
                        try:
                            ...
                            logger.info(f"Created fallback inverter summary view {summary_view_name} for {site_name}")
                        except Exception as e:
                            logger.error(f"Error creating fallback view: {e}")
